...
teacher_model, teacher_tokenizer, _ = load_t5_model_and_tokenizer(True, AssetPaths.T5_BOOKING_MODEL.value)
...
def get_student_model(teacher_tokenizer):
    ...
    tiny_config = T5Config(
        ...
        num_layers=4,      # Fewer encoder layers (T5-small has 6)
        num_decoder_layers=4,  # Reduce decoder layers
...
def load_data():
    # Load dataset
    dataset = load_dataset("json", data_files=get_path_to(AssetPaths.UPDATED_RAG_DATASET.value), streaming=True)

    # Apply preprocessing
    dataset = dataset.map(lambda example: {"input": reformat_text(example['input'])})
...
def train_student():
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            print(f"Epoch {epoch + 1}, Loss: {total_loss / 2077}")
