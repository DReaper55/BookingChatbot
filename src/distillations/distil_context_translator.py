...
teacher_model, teacher_tokenizer, _ = load_t5_model_and_tokenizer(True, AssetPaths.T5_CONTEXT_TRANSLATOR_MODEL.value)
...
def get_student_model(teacher_tokenizer):
    ...
    tiny_config = T5Config(
        ...
        num_heads=4,       # Fewer attention heads (T5-small has 8)
...
def load_data():
    # Load dataset
    dataset = load_dataset("json", data_files=get_path_to(AssetPaths.CONTEXT_TRANSLATOR_DATASET.value), streaming=True)

    # Apply preprocessing
    dataset = dataset.map(lambda example: {"input": f"translate conversation: {example['input']}"})
...
def train_student():
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            print(f"Epoch {epoch + 1}, Loss: {total_loss / 3200}")

        print(f"Epoch {epoch + 1}, Loss: {total_loss / 3200}")

    # Save model locally
    student_model.save_pretrained(get_path_to(AssetPaths.T5_DISTIL_CONTEXT_TRANSLATOR_MODEL.value))
    student_tokenizer.save_pretrained(get_path_to(AssetPaths.T5_DISTIL_CONTEXT_TRANSLATOR_MODEL.value))
