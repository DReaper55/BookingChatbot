...
teacher_model, teacher_tokenizer, _ = load_t5_model_and_tokenizer(True, AssetPaths.T5_SLOT_FILLER_MODEL.value)
...
def get_student_model(teacher_tokenizer):
    ...
    tiny_config = T5Config(
        ...
        d_ff=1026,          # Reduce feed-forward network size
...
def load_data():
    # Load dataset
    dataset = load_dataset("json", data_files=get_path_to(AssetPaths.SLOT_FILLER_DATASET.value))

    # Apply preprocessing
    dataset = dataset.map(lambda example: {"input": f"ask question: {example['input']}"})
