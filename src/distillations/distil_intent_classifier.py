...
teacher_model, teacher_tokenizer, _ = load_t5_model_and_tokenizer(True, AssetPaths.T5_INTENT_CLASSIFIER_MODEL.value)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def distillation_loss(student_logits, teacher_logits, temperature=2.0):
    """Computes knowledge distillation loss using KL-Divergence"""
...
def get_student_model(teacher_tokenizer):
    ...
    tiny_config = T5Config(
        d_model=128,       # Smaller hidden size (default in T5-small is 512)
        d_ff=512,          # Reduce feed-forward network size
        num_layers=3,      # Fewer encoder layers (T5-small has 6)
        num_decoder_layers=3,  # Reduce decoder layers
        num_heads=2,       # Fewer attention heads (T5-small has 8)
...
def preprocess_data(example):
    ...
    # labels = {k: v.squeeze(0).to(device) for k, v in labels.items()}
...
def load_data():
    # Load dataset
    dataset = load_dataset("json", data_files=get_path_to(AssetPaths.PROCESSED_RAG_DATASET.value))

    # Apply preprocessing
    dataset = dataset.map(extract_text_and_intent)
...
def train_student():
    ...
    # Save model locally
    student_model.save_pretrained(get_path_to(AssetPaths.T5_DISTIL_INTENT_CLASSIFIER_MODEL.value))
    student_tokenizer.save_pretrained(get_path_to(AssetPaths.T5_DISTIL_INTENT_CLASSIFIER_MODEL.value))
